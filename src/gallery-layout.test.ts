...
describe('gallery lookbook layout', () => {
  it('uses the same inset on every side of the lookbook and between stills', () => {
    expect(GALLERY_STILL_INSET_CLASS).toBe('p-10 sm:p-16');
    expect(GALLERY_STILL_GAP_CLASS).toBe('gap-16 sm:gap-24');
    expect(GALLERY_LOOKBOOK_CLASS.split(' ')).toContain('bg-white');
    ...
    expect(GALLERY_LIGHTBOX_PAD_CLASS).toBe('p-6 sm:p-10');
  });
